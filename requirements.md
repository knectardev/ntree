...
| `harmony_styles.js` | ~55 | Inner-voice harmony style registry (mid-range voicing templates with bullish/bearish/doji interval sets and MIDI ranges) | `HARMONY_STYLES` |
...
| `state.js` | ~250 | Shared state objects + small utilities (including harmonic-aware mode toggle state) | `musicState`, `audioState`, `ui`, `allAudioDropdowns`, `updateStatus()`, `midiToNoteName()`, `rhythmToDuration()`, `rhythmToDurationMs()` |
| `theory.js` | ~490 | Scale/chord math, regime detection, pattern detection, voice separation, chord label helpers, chord-aware tonal-context builder with structural/extension/avoid tiers | `updateRegimeFromPrice()`, `getScaleNotes()`, `getTonalContext()`, `getCurrentChordToneMods()`, `quantizeToChord()`, `nearestScaleNote()`, `nearestStructuralNote()`, `isAvoidNoteForTonalContext()`, `offsetScaleDegree()`, `nearestScaleNoteAbove()`, `updateVisiblePriceRange()`, `detectMelodicPattern()`, `getDynamicMidiRange()`, `getChordTonesInRange()`, `forceNoteDifference()`, `forceNoteDifferenceStrict()`, `ensureVoiceSeparation()`, `advanceProgression()`, `getChordLabel()`, `getChordSequence()`, `getChordComponentPCs()` |
...
| `conductor.js` | ~1170 | Animation loop (RAF), processSubStep orchestrator, visual note emission, chart scroll sync, replay integration, pattern override engine, chord event emission, harmonic safety/landing integration | `startAudioAnimation()`, `stopAudioAnimation()`, `pauseAudioAnimation()`, `resumeAudioAnimation()`, `processSubStep()`, `emitSubStepNote()`, `emitNoteEvent()`, `emitChordEvent()`, `generatePatternNote()`, `onBarAdvance()`, `hookIntoReplaySystem()`, `PLAYHEAD_POSITION`, `SUB_STEP_COUNT`, `SUB_STEP_SECONDS` |
...
Saved on every UI change. Restored on page load. **URL parameterization**: Current audio settings are reflected in the page URL via a single `?audio=` query param (base64url-encoded JSON). When the URL is copied and loaded in a fresh browser, audio settings are restored. Precedence: URL > localStorage > defaults. When adding new persistent audio parameters, the URL serialization must be updated (see `.cursor/rules/audio-url-params.mdc`). Includes: upper/lower wick settings (enabled, volume, instrument, rhythm, pattern, patternOverride, restartOnChord), inner harmony settings (`enabled`, `volume`, `instrument`, `rhythm`, `style`, `bodySensitivity`, `dojiThreshold`, `maxVoices`), drumVolume, drumNaturalRoom, drumGlowIntensity, drumKit (per-piece On/Off + level/decay for `kick`, `snare`, `hat`, `tom`, `ride`, `cajon`, `clap`, `log`, `tabla`, `timbale`, `misc`; legacy saved keys are mapped for compatibility), genre (internally keyed; user-facing label is "Scale"), `harmonicAwareScale` toggle, rootKey, chordProgression, bassLineStyle, drumBeat, displayNotes, chordOverlay, sensitivity, beatStochasticity, melodicRange, `slurAmount`, glowDuration, displayMode, panel open/closed states, speed (BPM). **Copy shareable URL** button in Playback Controls copies the current URL with audio params to clipboard.
...
- **Inner voices / harmony panel**: Dedicated sub-panel with enable toggle, volume, instrument, rhythm, harmony style, body sensitivity, doji threshold, and max voices. Playback uses an independent harmony sampler and style-registry-driven interval templates in the mid range.
- **Instruments**: MIDI soundfonts (FluidR3_GM via gleitz.github.io): harpsichord, synth lead, pipe organ, strings, flute, slide guitar (upper); acoustic bass, electric bass, synth pad, pipe organ (lower).
- **Music and Scale Settings**: Scale selection (Major/Natural Minor, Lydian/Phrygian (Raag), Dorian/Altered, Pentatonic (Major/Minor), Phrygian/Chromatic), **Harmonic-aware scale** checkbox (on/off chord-aware melodic pools), chord progression (classical, pop, blues, jazz, canon, fifties, old, bridge), **Bass Line Style** dropdown (Walking Bass, Bluegrass, Baroque Counterpoint, Motown, Reggae, Latin Tumbao, Afrobeat, Pop/Rock Melodic, Electronic/Synth, Minimal/Drone), **Drum Beat** dropdown (Simple, Minimal Jazz, Latin/Salsa, Reggaeton/Latin Trap, Folk-Country Shuffle, Indian Tabla, Afrobeat, Funk Pocket, Lo-Fi/Dilla, Brazilian Samba, Electronic House), root key (C through B), Note Labels toggle, Chord Overlay toggle.
- **Audio visual sync tuning**:
  ...
  - **Slur Amount** (0-1): Scales tie/legato tolerance for near-static notes. Low values tighten slurring; high values increase note-joining behavior.
...
- **Harmonic-aware tonal context** (in `theory.js` → `getTonalContext()`): When enabled, each sub-step builds a chord-aware pool from the selected scale and active chord, tagging notes as structural (chord tones), extensions, or avoid notes (minor-2nd-above-chord-tone clash) with genre-specific override hooks (Raag intervals).
- **Landing and avoid-note handling** (in `conductor.js`): Strong beats (sub-steps 0 and 8) resolve soprano notes to structural tones; avoid-note targets are gravity-shifted to nearest structural notes.
...
- With **Harmonic-aware scale** enabled, `scale_asc` / `scale_asc_desc` consume the chord-aware playable pool (not a static full-scale pool), so asc-scale test mode follows progression harmony.
...
**Rendering**
- When audio is **playing**, the main canvas in `07_render_and_interactions.js` reserves a **note axis** (52px) on the **left** of the plot for piano-keyboard-style note labels; `noteAxisW = audioActive ? 52 : 0` so plot width and layout adjust automatically.
...
- **Slur Amount control + Slide Guitar option**: Added `Slur Amount` slider in Audio Visual Sync Tuning (persisted as `audioState.slurAmount`) to control tie/slur tolerance during legato logic, and added `Slide Guitar` as a selectable upper-voice instrument (`slide_guitar`, mapped to steel-guitar soundfont) for slur-focused testing.
- **Slur intensity retune**: Updated `audio/conductor.js` slur/tie logic to use a steeper non-linear `slurAmount` response and to permit adjacent-note joins at high slur settings, so max slider values produce a clearly more legato result while low values remain staccato-like.
...
- **Harmonic-aware melody architecture**: Added chord-aware tonal context in `audio/theory.js` (`getTonalContext()`, structural/extension/avoid tagging, avoid-note helpers) and integrated it in `audio/conductor.js` pool construction. `Asc. Scale`/`Asc / Desc. Scale` pattern override modes now follow chord changes through the chord-aware playable pool; soprano strong-beat landing (sub-steps 0/8) and avoid-note gravity shift are enabled when `Harmonic-aware scale` is checked (persisted as `harmonicAwareScale`, default `true`).
...
- **Inner voices / harmony panel + playback**: Added a dedicated "Inner Voices / Harmony" UI sub-panel in `chart.html` with persisted controls (`enabled`, instrument, rhythm, style, volume, body sensitivity, doji threshold, max voices), introduced `audio/harmony_styles.js` registry, loaded an independent harmony sampler in `audio/engine.js`, and added body-driven mid-range harmony triggering in `audio/conductor.js` (doji/dyad/triad behavior constrained to style-defined MIDI ranges).
- **Harmony note-axis highlight**: Updated `07_render_and_interactions.js` so active harmony note events lightly pulse/highlight matching pitches on the left musical note axis, including stronger tick emphasis and temporary label reveal for currently played harmony tones.
- **Harmony event visibility fix**: Corrected inner-harmony duration calculation in `audio/conductor.js` to use numeric rhythm-based seconds (instead of multiplying a Tone duration string), which restores reliable harmony note triggering and ensures the left-axis harmony highlight appears during playback.
- **Harmony whole-note rhythm option**: Added `Whole (1/1)` to the Inner Harmony rhythm dropdown in `chart.html` and updated `audio/ui.js` load/runtime validation to persist and honor `rhythm: '1'`; playback already supports whole-note cadence via conductor rhythm interval/duration mapping.
- **Swing beat preset refresh**: Updated the `minimal_jazz` drum preset in `audio/drums2.js` to a more standard swing feel (ride skip pattern + hi-hat chick on 2/4 with lighter kick foundation), and renamed the corresponding Drum Beat dropdown label in `chart.html` to **Standard Swing Beat**.
...
- **Slur Amount dramatic legato pass**: Retuned `audio/conductor.js` so slur now strongly scales note duration with a nonlinear curve (very short/staccato at low values, much longer overlap at high values), while tie behavior remains same-pitch only to avoid cross-pitch retrigger skips; soprano gets a slightly higher legato duration cap for clearer slide-style phrasing.
- **Slide-guitar monophonic glide path**: Added a dedicated soprano slide lane in `audio/conductor.js` that activates for `slide_guitar` with higher `Slur Amount`, using a monophonic Tone voice with dynamic portamento (`setNote`) and reduced re-attack behavior; includes scoop-on-attack and lightweight vibrato/drive/compression chain for stronger continuous slide character while preserving sampler playback fallback.
- **Notes-aligned active-note slide logic**: Updated `audio/conductor.js` to maintain explicit soprano active-note state for slide phrasing, force-enable continuous glide behavior at high slur settings regardless of selected upper instrument, and treat active glide transitions as note extensions (not repeated fresh attacks) in the event emission path.
- **Slide expressivity humanization pass**: Refined `audio/conductor.js` slide lane with non-linear interval glide timing, post-slide micro-intonation overshoot/settle on detune, and vibrato staging (depth suppressed during glide then faded in after landing); also retuned MonoSynth timbre to pulse-width + higher filter resonance for a more nasal/metal-bar slide character.
- **Slide-chain stability and anti-static pass**: In `audio/conductor.js`, changed slide-mode sampler suppression from per-step `releaseAll()` calls to one-time suppression on slide entry, added limiter/headroom management for the slide synth chain, and canceled/rebuilt detune/vibrato automation schedules per glide update to reduce zipper/static artifacts during sustained playback.
- **Slide fail-safe + low-churn modulation pass**: Hardened `audio/conductor.js` slide lane with a 30s automatic sampler fallback on runtime slide errors, simplified modulation to avoid per-step automation buildup (steady vibrato depth during active slide), and reduced slide chain saturation/resonance to lower static/clipping risk during long playback runs.
- **Stable glide-control backend swap**: Replaced soprano slide pitch movement in `audio/conductor.js` from `setNote`-driven updates to explicit `frequency.cancelScheduledValues(...)` + `linearRampToValueAtTime(...)` ramps, with guarded fallback behavior, to improve long-run glide stability and prevent degradation/silence from repeated note-update failures.
- **Long-run playback hardening pass**: Added context auto-resume guarding in the conductor animation loop, periodic slide-voice lifetime reset protection, reduced slide-chain distortion drive, and audible tied-note continuity retriggers for soprano/bass so tie-heavy passages do not decay into silence during extended playback.
- **Notes-driven ghost-voice purge pass**: Updated `audio/conductor.js` slide phrase handling to hard-release the prior monophonic slide voice when starting a non-slide phrase, switched slide synth voicing toward the notes-proposed MonoSynth contour (sawtooth + slower envelope + wider filter envelope), and added a periodic every-4-bars sampler `releaseAll()` cleanup guard to reduce long-run voice accumulation/static risk.
- **High-slur glide-churn reduction**: Optimized `audio/conductor.js` slide updates to skip redundant same-pitch glide automation and throttle rapid consecutive glide-control writes (~40ms minimum spacing), reducing audio-thread load when `Slur Amount` is high while preserving continuous slide phrasing.
- **Drum local-probe 404-noise suppression**: Updated `audio/drums2.js` to run in fallback-only mode by default (no local sample URL probing) unless explicitly enabled via `audioState.enableLocalDrumProbe`, eliminating repeated console 404s when `static/demo_static/audio/drums/*` folders do not contain one-shot files.
- **Strict soprano monophonic gating pass**: Updated `audio/conductor.js` to enforce release-before-attack on non-slide phrase starts (with a tiny attack offset), force slide-voice release when soprano pulse gating says “don’t play,” and tighten slide-chain peak control with a stronger limiter threshold (`-6 dB`) to reduce tail stacking/static risk during high-slur playback.
- **Non-slide stability rollback pass**: Removed global every-4-bars sampler `releaseAll()` sweeps and removed tie-path continuation retriggers for soprano/bass in `audio/conductor.js` (ties now only extend visual/event duration), reducing extra overlapping note generation that could destabilize non-slide instruments like flute.
- **Dynamic-control panic-flush hook**: Added `panicFlushVoices()` export in `audio/conductor.js` and wired debounced live-slider flushes in `audio/ui.js` for key tuning controls (`Complexity`, `Beat Stochasticity`, `Pattern Density`, `Flow/Sustain`, `Slur Amount`, `Melodic Range`) so rapid parameter ramping clears lingering voices and reduces static/crackle from transient voice pile-up.
